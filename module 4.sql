...
UPDATE orders SET Status = "Delivered" WHERE order_id = 1004;
UPDATE orders SET Status = "Delivered" WHERE order_id = 1005;
UPDATE orders SET Status = "Confirmed" WHERE order_id = 1006;
UPDATE orders SET Status = "Confirmed" WHERE order_id = 1007;
...
-- Removing the null status orders
select coalesce(Status, 'Not Specified'), count(order_id) from orders group by Status;

select payment_method, payment_date, count(payment_id) from payment group by payment_method, payment_date;
...
select count(order_id), order_date, sum(total_amount) from orders group by order_date;

select customer_id,count(order_id) from orders group by customer_id HAVING count(order_id) > 1;

SELECT product_id, quantity, COUNT(*)
FROM order_items
